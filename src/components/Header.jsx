...
export default function Header({ onToggleCart, cartCount, onSearch, searchTerm, onOpenMenu }) {
  return (
    <header className="bg-stone-800 text-amber-400 sticky top-0 z-30">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex justify-between items-center py-4">
          <div className="flex items-center gap-4">
            <button className="md:hidden p-2" onClick={onOpenMenu} aria-label="menu">
            ...
            <div className="flex items-center">
              <img src={Logo} alt="Logo" className="h-10 w-auto sm:h-12 md:h-14 object-contain" />
            </div>
          </div>

          <div className="flex-1 mx-8 hidden md:block">
            <div className="relative">
              <input value={searchTerm} onChange={(e) => onSearch(e.target.value)} className="w-full border rounded-md px-4 py-2 focus:outline-none focus:ring-2 focus:ring-amber-300 bg-stone-700 text-amber-200" placeholder="Search products, categories..." />
              <button className="absolute right-2 top-1/2 transform -translate-y-1/2 text-amber-200">🔍</button>
            </div>
          </div>

          <div className="flex items-center gap-4">
            <button className="hidden sm:inline text-sm px-3 py-2 rounded-md hover:bg-stone-700">Collections</button>
            <button className="hidden sm:inline text-sm px-3 py-2 rounded-md hover:bg-stone-700">About</button>

            <button onClick={onToggleCart} className="relative p-2 rounded-md hover:bg-stone-700">
              <svg className="h-6 w-6 text-amber-300" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M3 3h2l.4 2M7 13h10l4-8H5.4M7 13L5.4 5M7 13l-2 6 m12-6l2 6m-6 0a1 1 0 11-2 0 1 1 0 012 0 m-6 0a1 1 0 11-2 0 1 1 0 012 0" />
              </svg>
              {cartCount > 0 && <span className="absolute -top-1 -right-1 bg-red-500 text-white text-xs rounded-full px-1">{cartCount}</span>}
            </button>

            <button className="px-3 py-2 bg-amber-400 text-stone-800 rounded-md hidden md:inline">Sign in</button>

            <Link to="/admin" className="px-3 py-2 border rounded hidden md:inline text-amber-200">Admin Login</Link>
          </div>
        </div>
      </div>
    </header>
